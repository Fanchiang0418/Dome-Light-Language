...
// ====== 背景模型 ======
PShape bgModel;
PVector bgPos = new PVector(width/2, height*0.65, -400); // 先給個大概
float bgScale = 1.0;
float bgRotX = 0;
float bgRotY = 0;
float bgRotZ = 0;
...
float lightHeight = 150;   // 每根燈柱的長度（沿射線方向）
int numSegments = 14;      // 每條燈切成幾個 pixel
...
void setup() {
  ...
  // ====== 載入背景模型（把檔案放到 sketch 的 data 資料夾）======
  // 例：data/bg.obj
  bgModel = loadShape("dome2.obj");
  // bgModel.disableStyle(); // 如果你想完全用你自己的 fill/stroke，就打開這行

  // 重新設定 bgPos（這裡才拿得到 width/height）
  bgPos.set(width/2, groundY - 60, 0);  // -60 先把模型抬起來避免穿地
...
void draw() {
  ...
  // ===== 先畫背景模型（在同一套相機座標裡）=====
  if (showModel && bgModel != null) {
    drawBackgroundModel(g);
  }
...
void keyPressed() {
  // ===== 微調模型位置/大小 =====
  float step = 20;          // 位移步長
  float rotStep = 0.08;     // 旋轉步長

  if (key == 'm' || key == 'M') showModel = !showModel;

  // 平移：J/L 左右、I/K 上下、U/O 前後
  if (key == 'j' || key == 'J') bgPos.x -= step;
  if (key == 'l' || key == 'L') bgPos.x += step;
  if (key == 'i' || key == 'I') bgPos.y -= step;
  if (key == 'k' || key == 'K') bgPos.y += step;
  if (key == 'u' || key == 'U') bgPos.z += step;
  if (key == 'o' || key == 'O') bgPos.z -= step;
  ...
  // 旋轉：R/T 繞Y
  if (key == 'r' || key == 'R') bgRotY -= rotStep;
  if (key == 't' || key == 'T') bgRotY += rotStep;
