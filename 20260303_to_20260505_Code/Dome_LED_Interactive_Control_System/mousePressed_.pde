...
void mousePressed() {
  ...
  // 原本：抓 ribbon pixel（用螢幕距離）
  selectedPixel = pickRibbonPixelByScreen(mouseX, mouseY, pickRadiusScreen);
  if (selectedPixel != -1) println("Picked ribbon pixel = " + selectedPixel);
}
